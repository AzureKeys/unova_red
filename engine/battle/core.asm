...
.calcCriticalHitProbability
	...
	ld hl, HighCriticalMoves     ; table of high crit moves
.Loop
	ld a, [hli]                  ; read move from table
	cp c                         ; does it match move being used?
	jr z, .HighCritical          ; if so, we're using a high crit move
	inc a                        ; move to next in the list, FF terminates
	jr nz, .Loop                 ; go to next
